...
// 1. Obtener los seguros vencidos de la base de datos
// Filtramos por seguros cuyo estado sea 'Vencido' y que no estén borrados.
$db = new Model_DB();
$conn = $db->connect();
$stmt = $conn->prepare("
    SELECT 
        ...
        fecha_creacion
    FROM seguros 
    WHERE estado = 'Vencido' AND deleted = 0
    ORDER BY fecha_creacion DESC
");
$stmt->execute();
$seguros_vencidos = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
$html = '
    ...
    <h1>Reporte de Seguros Vencidos</h1>';

if (empty($seguros_vencidos)) {
    $html .= '<p style="text-align: center; margin-top: 50px;">No hay seguros vencidos en este momento. 🎉</p>';
} else {
    $html .= '
    <table>
        <thead>
    ...
    foreach ($seguros_vencidos as $seguro) {
...
$mpdf->Output('reporte_seguros_vencidos.pdf', 'I');
exit;